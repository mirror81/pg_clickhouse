...
#define RD_FIXED(suffix, T)                                                            \
    static inline T rd_##suffix(const uint8_t* p, uint64_t row) {                      \
        T v;                                                                           \
...
static inline void
slice_str(const chc_column* col, uint64_t row, const char** out_ptr, size_t* out_len) {
...
read_array(
    ...
    if (len > 0) {
        ...
        for (uint64_t i = 0; i < len; ++i) {
            slot->datums[i] =
                read_value(inner, inner_t, start + i, &scratch, &slot->nulls[i]);
        }
    } else {
        slot->datums = NULL;
        slot->nulls  = NULL;
    ...
    case CHC_UINT32:
        *valtype = INT8OID;
        return Int64GetDatum(
            (int64)rd_u32((const uint8_t*)chc_column_fixed_data(col, NULL), row)
        );
    case CHC_INT64:
        *valtype = INT8OID;
        return Int64GetDatum(
            rd_i64((const uint8_t*)chc_column_fixed_data(col, NULL), row)
        );
    case CHC_UINT64: {
    ...
    case CHC_FLOAT32:
        *valtype = FLOAT4OID;
        return Float4GetDatum(
            rd_f32((const uint8_t*)chc_column_fixed_data(col, NULL), row)
        );
    case CHC_FLOAT64:
        *valtype = FLOAT8OID;
        return Float8GetDatum(
            rd_f64((const uint8_t*)chc_column_fixed_data(col, NULL), row)
        );
    case CHC_DECIMAL32:
    case CHC_DECIMAL64:
    case CHC_DECIMAL128:
    ...
    case CHC_DATE:
        *valtype = DATEOID;
        return DateADTGetDatum(
            (DateADT)rd_u16((const uint8_t*)chc_column_fixed_data(col, NULL), row) -
            CH_TO_PG_DATE_OFFSET
        );
    case CHC_DATE32:
        *valtype = DATEOID;
        return DateADTGetDatum(
            (DateADT)rd_i32((const uint8_t*)chc_column_fixed_data(col, NULL), row) -
            CH_TO_PG_DATE_OFFSET
        );
    case CHC_DATETIME: {
